...
public static class FileLauncherTool
{
    ...
    public static async Task<StartResult> Start(
        ...
        [Description($"The ID of the binary file to {nameof(StartMode)}.{nameof(StartMode.ExecuteShellBinary)} or {nameof(StartMode)}.{nameof(StartMode.LaunchGraphicalApplication)} per {nameof(fileIdStartMode)}. This MUST be a file (never folder). This MUST NOT contain any {nameof(processArguments)}. File is copied to local storage if non-local file is given. Protocol scheme aliases are supported and encouraged for brevity.")] string fileId,
        [Description("Arguments passed to the started process. Storage aliases embedded inline are resolved before launch, e.g. \"diff -- customalias://relative/path/file.md\".")] string processArguments = "",
        [Description("Working directory for the process to start in. May be any locally browse-able protocol alias; aliases are resolved to full paths before launch.")] string? processWorkingDirectory = null,
